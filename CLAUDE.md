...
### 5. Middleware 작성 규칙 (⚠️ 매우 중요)

> **Vercel 빌드 경고 방지를 위한 필수 규칙**

#### 파일 위치 (필수)
```
✅ 올바른 위치: 프로젝트 루트/middleware.ts
❌ 잘못된 위치: src/middleware.ts, src/proxy.ts
```

#### 절대 금지 사항
```typescript
// ❌ src/proxy.ts 사용 금지 (Next.js 16 deprecation)
// ❌ src/middleware.ts 사용 금지 (Vercel 빌드 경고 발생)
// ❌ @supabase/auth-helpers-nextjs 사용 금지 (deprecated)
// ❌ middleware.ts와 proxy.ts 동시 존재 금지
```
...
| 항목 | 설명 |
|------|------|
| **파일 위치** | 반드시 프로젝트 루트 (`/middleware.ts`) |
| **패키지** | `@supabase/ssr` 사용 (auth-helpers 아님) |
| **함수명** | `export async function middleware` |
| **config** | `matcher` 패턴으로 적용 경로 지정 |
